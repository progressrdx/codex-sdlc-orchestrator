...
6. When an active workflow exists, let short requests such as “继续”, “查看进度”, “修改需求”, or “开始验收” resume or inspect that workflow. Without an active workflow, those phrases alone do not start one.
   If the user explicitly pauses the workflow, run `pause --reason ...`; paused state preserves evidence, stays silent for unrelated prompts, selectively routes explicit continue/status requests, and rejects delivery mutations. Run `resume` only when the user asks to continue.
