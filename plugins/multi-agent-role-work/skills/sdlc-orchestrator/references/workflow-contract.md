...
Every response routed through this Skill begins with the exact visible identity block `### Project Compass` followed by `**项目守航已开启**`. This is the user's deterministic activation signal. Never omit or paraphrase it, and never present an ordinary response as plugin-owned when the signal is absent. A Skill-only identity block is branded conversation content, not native host-rendered plugin UI.

The internal workflow starts only after the user explicitly starts a project with a concrete goal. Recommended discovery entries include `开始一个新项目`, `开启一个新项目`, `创建新项目`, `我想做一个项目`, and `帮我做一个项目`; when no goal is included, show the activation signal, ask what the user wants to accomplish, and do not initialize state yet. An actionable `开始项目：<goal>` or equivalent request to keep a named goal moving without drifting starts the project; the goal text becomes the original request and opens discovery only. Legacy `团队开发：<request>` prompts, explicit multi-role process requests, and direct `$sdlc-orchestrator` invocation remain advanced compatibility entries. Merely discussing project management does not activate the workflow. An active project continues when `.ai-workflow/active.yaml` exists and the user asks to continue, inspect alignment or progress, change the goal, or confirm the result. Without active state, short continuation phrases do not create a project. Ordinary questions and isolated edits stay outside the workflow.
...
On every later prompt explicitly routed to an active project, read `overview --json` internally before delegating or editing, then use `project` for the normal user-facing update. Lead with whether the project remains aligned with its recorded goal. `overview` and `status` are advanced diagnostic interfaces; SDLC, stage, mode, gate, role, evidence, meeting, and budget terminology should not appear in the default response. Routing is on demand through clear phrases such as “继续推进当前项目”, “项目有没有跑偏”, “查看项目进展”, “调整项目目标”, or “确认最终结果”; legacy phrases remain compatible. A bare “继续” is ordinary conversation because the Skill cannot inspect active state before it is selected; no active pointer means even a clear continuation phrase reports no project and never creates one. The plugin installs no lifecycle hooks; ordinary conversation never runs workflow code.
