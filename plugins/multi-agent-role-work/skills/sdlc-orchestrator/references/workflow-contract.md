...
On every later prompt explicitly routed to an active requirement, read `overview` before delegating or editing. Routing is on demand through `$sdlc-orchestrator` or clear phrases such as “继续团队开发”, “查看研发进度”, “修改当前需求”, or “开始验收”. A bare “继续” may inspect an active workflow but never resumes a paused workflow; no active pointer means it remains ordinary conversation. The plugin installs no lifecycle hooks; ordinary conversation never runs workflow code.

`pause` keeps the pointer, stage, and evidence and rejects all delivery mutations until `resume`. Resume only from an explicit formal-workflow request such as “继续团队开发”; a bare “继续” may return `overview` but must not change state. Unrelated prompts remain ordinary conversation because no lifecycle hook is registered. Use pause when the user temporarily switches topics; do not complete or discard a requirement merely to avoid context overhead.
...
At each configured gate, every required role must submit an explicit verdict and a distinct stable task/session reference. Prefer `submit-gate-review`, which validates the complete role set, unique evidence and actor references, verdict consistency, participant coverage, and meeting evidence before one state write. A rejected bundle leaves the existing gate state unchanged. Granular `decide` and `record-meeting` remain recovery interfaces. The references are included in meeting and human-approval snapshots so accidental role reuse is rejected. A rejection is not averaged away.
...
State files use a versioned schema, semantic checksum, monotonic revision, atomic replacement, a prior-valid-state backup, and a repository-scoped cross-process writer lock. Delivery safety comes from the workflow state machine, independent reviews, source verification, and explicit user confirmation rather than a lifecycle hook. Persistence and locking live in `state_store.py`; argparse definitions live in `workflow_cli.py`; cost-aware command execution and bounded logs live in `execution_policy.py`; shared module binding lives in `command_runtime.py`; risk and escalation handlers live in `risk_commands.py`; gate reviews and human approvals live in `review_commands.py`; protected baselines and verification live in `assurance_commands.py`; feedback, delivery confirmation, and issue handling live in `delivery_commands.py`; shared workflow rules and compatibility wrappers remain in `workflow.py`. Unsupported direct edits fail closed; use workflow commands. `audit-state` remains available when the main state checksum or syntax is damaged, and `repair-state --from-backup --confirm RESTORE` restores only a validated backup with a new revision and audit entry. A stale writer is rejected when the on-disk revision differs from the revision it loaded. These controls protect local consistency; they do not resolve Git merge conflicts or provide a cryptographic identity for reviewers.
