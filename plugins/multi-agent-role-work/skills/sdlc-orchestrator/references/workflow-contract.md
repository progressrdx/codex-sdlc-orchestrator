...
Micro, quick, and standard verification reports bind the current tracked and untracked workspace content while excluding workflow state and requirement evidence. A later product-file change makes that report stale at acceptance and delivery, and the same report file cannot simply be re-registered after a rewind. Strict retains its stronger committed, reviewed-scope Git binding and checks it through final delivery confirmation.
...
State files use a versioned schema, semantic checksum, monotonic revision, atomic replacement, a prior-valid-state backup, and a repository-scoped cross-process writer lock. The plugin also bundles a reviewed lifecycle hook: outside `prototype` and `implementation`, `apply_patch` product edits are denied while workflow evidence remains writable. This is a strong guard against accidental coordinator bypass, not a security sandbox; specialized tools and shell side effects still require instruction compliance. Persistence and locking live in `state_store.py`; argparse definitions live in `workflow_cli.py`; shared module binding lives in `command_runtime.py`; risk and escalation handlers live in `risk_commands.py`; gate reviews and human approvals live in `review_commands.py`; protected baselines and verification live in `assurance_commands.py`; feedback, delivery confirmation, and issue handling live in `delivery_commands.py`; shared workflow rules and compatibility wrappers remain in `workflow.py`. Unsupported direct edits fail closed; use workflow commands. `audit-state` remains available when the main state checksum or syntax is damaged, and `repair-state --from-backup --confirm RESTORE` restores only a validated backup with a new revision and audit entry. A stale writer is rejected when the on-disk revision differs from the revision it loaded. These controls protect local consistency; they do not resolve Git merge conflicts or provide a cryptographic identity for reviewers.
