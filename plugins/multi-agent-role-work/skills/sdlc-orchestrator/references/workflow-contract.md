...
Use `overview --json` for internal handoffs and deterministic progress checks. Use `project` for user communication; it reports the goal, core-result states, current focus, recent outcomes, inspectable previews/reports, resolved problems, quality, and decisions needing the user without exposing the internal workflow vocabulary. Result states must be derived from recorded goals, artifacts, verification, and outcomes; never manufacture a completion percentage.
